...
void main() {
  ...
  Future<HarfArabalariOyunuState> open(
    ...
    return tester.state<HarfArabalariOyunuState>(find.byType(HarfArabalariOyunu));
  }
  ...
  Future<HaCar> waitTargetCar(WidgetTester tester, HarfArabalariEngine e) async {
    for (var i = 0; i < 120; i++) {
      final c = e.cars.where((c) =>
          c.catchable &&
  ...
  Future<void> finish(WidgetTester tester, HarfArabalariEngine engine,
      {bool perfect = false}) async {
    while (engine.status == HaStatus.running) {
      engine.tick(0.1);
      if (perfect) {
        final t = engine.cars.where((c) =>
            c.catchable && c.letter.char == engine.target?.char && c.x > 0);
  ...
  testWidgets('başlangıç: kısa açıklama, oyuncu etiketi ve BAŞLA', (tester) async {
    final state = await open(tester);
    expect(find.text('Harf Arabaları'), findsWidgets);
    expect(find.text('Sesi dinle, doğru harfli arabayı yakala!'), findsOneWidget);
    expect(find.text('BAŞLA'), findsOneWidget);
    expect(find.text('Oyuncu seç'), findsOneWidget);
    expect(state.engineForTest!.status, HaStatus.ready);
  });

  testWidgets('BAŞLA: süre, hedef ses, arabalar soldan sağa geçer', (tester) async {
    final state = await open(tester);
    await start(tester);
  ...
  testWidgets('yanlış araba: kaybolmaz, puan düşmez, hedef aynı, tekrar denenir',
      (tester) async {
    final state = await open(tester);
    await start(tester);
    final engine = state.engineForTest!;
    Iterable<HaCar> visibleWrong() => engine.cars.where((c) =>
        c.catchable &&
  ...
  testWidgets('web/masaüstü: fare tıklamasıyla araba yakalanır', (tester) async {
  ...
  testWidgets('arka plana alınca oyun/süre durur, ses susar; dönünce devam eder',
      (tester) async {
    final state = await open(tester);
    await start(tester);
    final engine = state.engineForTest!;
  ...
  testWidgets('120 sn dolunca sonuç: puan, doğru, geçen araba, kaçırılan, yanlış, doğruluk, en iyi',
      (tester) async {
    final state = await open(tester);
    await start(tester);
    final engine = state.engineForTest!;
  ...
  testWidgets('sonuç aktif oyuncuya yazılır; başka oyuncunun skoru değişmez',
    ...
    await tester.runAsync(() async {
      ...
      await repo.recordResult(PlayerGameResult(
        playerId: ahmet.id,
    ...
    expect(repo.bestScore(ahmet.id, GameIds.harfArabalari), 999,
        reason: 'Ahmet\'in skoru Elif\'in oyunundan etkilenmez');
    expect(find.text('Elif'), findsWidgets, reason: 'sonuç panelinde oyuncu adı');
  });

  testWidgets('aktif oyuncu yoksa BAŞLA bir kez "Kim oynuyor?" sorar; profil sonra hatırlanır',
      (tester) async {
    late PlayerRepository repo;
    await tester.runAsync(() async {
      repo = PlayerRepository();
    ...
    expect(state.engineForTest!.status, HaStatus.running, reason: 'profilden sonra başlar');
        ...
        body: Center(
          child: ElevatedButton(
            onPressed: () => Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => HarfArabalariOyunu(random: math.Random(2)))),
  ...
  testWidgets('oyun listesinde Harf Arabaları ve Skor Tablosu kartları; oyuncu etiketi',
      (tester) async {
    useSize(tester, const Size(390, 844));
